...
import {REGION_SWITCH,SITE_SWITCH,JOURNEY_END,STILL_VIEWS,chapterAt} from './timeline.ts';
...
function distanceSchedule(path:CatmullRomCurve3){
 ...
 return(p:number)=>{const i=Math.max(0,Math.min(siteKnots.length-2,siteKnots.findIndex(x=>p<x)-1));
  if(p>=JOURNEY_END)return 1;const span=siteKnots[i+1]-siteKnots[i];return hermite(ys[i],ys[i+1],ms[i]*span,ms[i+1]*span,clamp((p-siteKnots[i])/span));};
}
...
export type JourneyShot=Omit<Shot,'scene'> & {scene:'solar'|'earth'|'region'|'site';up:Vec3;guidePath:'solar'|'orbital'|'region'|'site';guideU:number;trailLength:number;cloudOpacity:number;regionEmphasis:number;guideScale:number};
...
export function journeyCopy(p:number,still=false){
 if(still){const index=Object.values(STILL_VIEWS).findIndex(value=>value===p);if(index>=0){const values=Array(7).fill(0);values[[1,3,4,5,6][index]]=1;return values;}}
 const first=acceptedCopy(p);return [...first,
 Math.max(fadeWindow(p,1.025,1.07,1.20,1.24),fadeWindow(p,1.345,1.375,1.40,1.43)),
 fadeWindow(p,1.65,1.69,1.77,1.83),fadeWindow(p,2.06,2.10,2.16,2.21)];
}
export function sampleJourney(progress:number,mode:Framing='landscape'):JourneyShot{
 const p=clamp(progress,0,JOURNEY_END),accepted=sampleAccepted(Math.min(1,p),mode);
 ...
 if(p>=SITE_SWITCH){
  const t=clamp((p-SITE_SWITCH)/(JOURNEY_END-SITE_SWITCH));
