...
/**
 * Tyl's own guide documents a DIFFERENT notification hash to Fiserv's:
 * a legacy unsorted concatenation with the secret inside the string and no
 * pipes. Both are published as current by their respective owners, so we
 * accept either rather than silently dropping a real payment.
 */
export function legacyNotificationHash(
  { chargetotal, currency, txndatetime, storename, approval_code }, secret) {
  const str = `${chargetotal}${secret}${currency}${txndatetime}${storename}${approval_code}`;
