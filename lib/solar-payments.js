...
export const POLICY_VERSION='2026-09-09-stripe-v2';
...
export function validateApplication(body){
 ...
 // Documented applicant confirmations are retained for operator review. They
 // are never a certification of the installation or network permission.
 const screening={};
 for(const key of ['existing','units','tested','phase']){
  if(body.screening?.[key]!=='yes')throw new PaymentError('suitability_review',422);
  screening[key]='yes';
 }
 ...
 return {...details,screening,agree:true,privacy:true,acceptedTerms:true,acceptedPrivacy:true};
}
...
export function requestOrigin(req,cfg){
 const allowed=cfg.livemode?['https://aesir-solar.vercel.app','https://aesirsolar.co.uk','https://www.aesirsolar.co.uk']:[cfg.origin];
 if(!allowed.includes(req.headers.origin))throw new PaymentError('invalid_origin',403);
