...
const folder='docs/experience/stage-two/captures';await mkdir(folder,{recursive:true});
...
const frames=[['opening',0],['approach',.22],['close',.36],['acquisition',.455],['early-flight',.535],['mid-flight',.665],['earth-reveal',.84],['earth-hold',.935]];
const subset=process.env.EXPERIENCE_CAPTURE_VIEWS?.split(',');
const reports=[];
for(const [name,viewport] of viewports.filter(v=>!subset||subset.includes(v[0]))) {
 ...
 for(const [label,p] of frames) {
  await page.evaluate(p=>{const j=document.querySelector('#journey');window.scrollTo(0,p*(j.offsetHeight-innerHeight));},p);
  if(p>.25)await page.waitForFunction(()=>['ready','failed'].includes(window.__experience?.snapshot().earthStatus)||window.__experience?.snapshot().failed);
  await page.waitForTimeout(400);await page.screenshot({path:`${folder}/${name}-${label}.png`});
...
await writeFile('docs/experience/stage-two/capture-observations.json',JSON.stringify({browser:browser.version(),reports},null,2));await browser.close();console.log(reports.map(r=>({name:r.name,errors:r.errors,states:r.states.map(s=>({frame:s.label,progress:s.progress,earth:s.earthStatus,failed:s.failed,trailHeadError:s.trailHeadError}))})));
