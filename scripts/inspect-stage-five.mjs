...
const moments=[['opening',0],['sun',.24],['flight',.55],['earth',.935],['panel',2.12],['absorption',2.68],['inverter',4.02],['operating-note',4.20],['business',4.50],['screen',4.65],['storage-charge',5.07],['storage-later',5.33],['grid',5.53],['connection',5.86],['brand',6.03]];
const stillViews=[['sun',.285],['earth',.925],['britain',1.4],['roof',1.76],['panel',2.23],['cell',2.76],['dc',3.40],['inverter',4.02],['business',4.70],['storage',5.09],['connected',5.56],['aesir',6.03],['inverter',4.02],['panel',2.23]];
...
async function awaitChapter(page,p,still=false) {
  await page.waitForFunction(({p,still})=>{
    ...
    return s.ready&&(still||Math.abs(s.progress-p)<.003)&&(p<=.635||s.earthStatus==='ready')&&(p<=1.235||s.regionStatus==='ready')&&(p<=1.49||s.siteStatus==='ready')&&(p<=2.34||s.cellStatus==='ready')&&(p<=3.05||s.electricalStatus==='ready')&&(p<=4.18||s.businessStatus==='ready')&&(p<=4.96||s.storageStatus==='ready');
...
async function layout(page) {
  return page.evaluate(()=>{
    ...
    return {framing:document.body.dataset.framing,largeType:document.body.dataset.largeType,width:document.documentElement.scrollWidth,viewport:{width:innerWidth,height:innerHeight},activeElement:{tag:document.activeElement?.tagName,id:document.activeElement?.id,text:document.activeElement?.textContent},
      ...
      annotations:[...document.querySelectorAll('[data-inverter-label],[data-storage-label],.process-note')].filter(visible).map(describe),
...
try {
  ...
  for(const [name,width,height,dpr] of [['zoom-200',640,360,2],['text-200',1280,720,1]]) {
    ...
    try {
      ...
      if(name==='text-200')await page.evaluate(()=>{
        ...
        const sizes=[...document.querySelectorAll('h1,h2,h3,p,button,a,li,[data-inverter-label],[data-storage-label],#scroll-label')].map(e=>[e,parseFloat(getComputedStyle(e).fontSize)]);
  ...
  // All twelve still chapters, including a backwards revisit after electrical load.
  for(const [name,width,height,dpr] of [['laptop',1280,720,1],['mobile',390,844,3],['short',1000,500,1]]) {
  ...
  const recording={segments:[[0,4.08,60000],[4.08,6.08,42000],[6.08,4.02,36000]],samples:[]};observations.video=recording;
  ...
  try {
    await guard(page,recording);await ready(page);await scroll(page,0);await page.waitForTimeout(600);
  ...
  console.log('Stage-five zoom, text, still, keyboard and video observations written to',root);
