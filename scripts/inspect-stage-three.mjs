...
const browser=await chromium.launch(launchOptions),root='docs/experience/stage-three';await mkdir(`${root}/captures`,{recursive:true});
const results=[];
...
for(const [name,width,height,dpr] of [['zoom-200',640,360,2],['text-200',1280,720,1]]){
 ...
 const states=[];
 for(const [label,p]of [['opening',0],['sun',.24],['flight',.55],['earth',.935],['britain',1.4],['building',1.72],['panel',2.12]]){
  await page.evaluate(p=>{const j=document.querySelector('#journey');scrollTo(0,p/Number(j.dataset.duration)*(j.offsetHeight-innerHeight));},p);if(p>.16)await page.waitForFunction(p=>{const s=window.__experience.snapshot();return s.earthStatus==='ready'&&(p<1.235||s.regionStatus==='ready')&&(p<1.49||s.siteStatus==='ready');},p);await page.waitForTimeout(200);
  await page.screenshot({path:`${root}/captures/${name}-${label}.png`});
  states.push(await page.evaluate(()=>({progress:window.__experience.snapshot().progress,framing:document.body.dataset.framing,width:document.documentElement.scrollWidth,viewport:innerWidth,visible:[...document.querySelectorAll('[data-copy]')].filter(e=>parseFloat(e.style.opacity)>.15).map(e=>({text:e.textContent,rect:e.getBoundingClientRect().toJSON(),bottom:Math.max(...[...e.children].map(x=>x.getBoundingClientRect().bottom)),footer:document.querySelector('.chapter-footer').getBoundingClientRect().top}))})));
 }
...
for(const [from,to,duration]of [[0,2.25,35000],[2.25,.925,22000],[.925,2.25,22000]]){
 await page.evaluate(async({from,to,duration})=>{const j=document.querySelector('#journey'),distance=(j.offsetHeight-innerHeight)/Number(j.dataset.duration);await new Promise(resolve=>{const start=performance.now();function frame(now){const t=Math.min(1,(now-start)/duration);scrollTo(0,(from+(to-from)*t)*distance);if(t<1)requestAnimationFrame(frame);else resolve();}requestAnimationFrame(frame);});},{from,to,duration});await page.waitForTimeout(600);
}
