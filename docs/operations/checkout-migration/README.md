...
- No dedicated Solar database is configured in the candidate. Read-only connected Supabase project inventory found no Solar project. The user selected the existing AESIR organisation for a dedicated Solar database. Supabase confirms AESIR is on Pro and quotes US$10/month base cost for an additional project. Proposed project: `aesir-solar`, London (`eu-west-2`). Cost acceptance is pending; no project has been created.
