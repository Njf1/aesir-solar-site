...
## Operational completion candidate — 9 September 2026

- Vercel Pro is independently verified active. User expressly approved the production-only credential installation, £300 Stripe activation, this deployment and root/www domain move. Credentials remain private; no keys in Git/client output. Stripe account and live VAT checks passed, including read access to Payment Intents, charges, balance and payouts. Live webhook `we_1UDmQoL9cBVV8DnXZ6Jfls9r` targets the stable Vercel API URL with Checkout completed/async-success events.
- Original Fasthosts `hello` mailbox authenticated over verified TLS on port465. User-approved customer-data-free test mail was accepted, and the user confirmed arrival in Inbox. This proves that specific send; future delivery still depends on providers.
- `solar_alerts` is server-only. A trigger enqueues in the same transaction that creates a paid work item. Claims use row locks/SKIP LOCKED, mode isolation, expiring leases and exponential retry. Only SMTP acceptance followed by durable acknowledgement marks sent. Stable Message-ID/reference reduces confusion after an uncertain SMTP acknowledgement; SMTP cannot promise exactly-once email. The unique database work item remains exactly one.
- The private `/api/operations` worker runs every10 minutes in production: bounded Stripe reconciliation with a durable cursor, then pending paid alerts. Cron access requires a secret bearer token; HEAD checks live provider/database/mail readiness without sending mail. No customer endpoint can invoke it anonymously. The existing manual reconcile command covers larger forensic recovery.
- Encrypted-backup automation was proposed and **not authorised** after the user selected application alerts only. The cloud backup/backup cron was not created and no emailed backup was sent. Existing manual encrypted backup/restore tooling remains. Snapshot v2 now includes alert state; restore should clear transient leases. Supabase Free still has no automatic database backups and can pause under its plan rules. Do not describe this as managed disaster recovery.
- All original19 named input attributes and both consents are retained. Four additional un-named, accessible suitability controls are stored in a nested `screening` object: existing premises/all generation accounted for; each unit intrinsic capacity≤32A; appropriate type testing; documented balanced per-phase/export arrangement. Unknowns fail before payment on both client and server. These are applicant confirmations, not engineering certification. Operator review and document verification remain part of the paid service. The conservative original16/32/60A boundary review rules remain.
- Return URLs use the initiating allowlisted Solar origin, preventing a migration from losing the browser's private receipt token. Other origins and test-to-live crossover fail closed.
- Nodemailer10.0.1 is the one new pinned server-only dependency. All existing dependency/lock entries remain unchanged; it is excluded from static delivery. No film source changed. New WebKit form selectors use explicit appearance and48px targets after visual review found native controls shrinking.
- Before final deployment:140/140 Node tests, typecheck/build passed;23/23 focused browser checks passed. Ten Chromium/WebKit form viewport captures had no overflow/errors (1280×720,1600×1000,390×844,740×900,1000×500). A subsequent small control-style correction is subject to the final recorded rerun below. Desktop Apple M4 browser emulation, not a new real-phone/Safari/Firefox certification. Historical full-film tests remain historical.

Primary sources accessed9 September2026: [Stripe testing](https://docs.stripe.com/testing) explicitly says not to use real card details/live mode to test. Use sandbox evidence; no manufactured real charge is needed. Genuine settlement can only be observed on a genuine later purchase. [ENA G99 Issue2](https://dcode.org.uk/assets/250307ena-erec-g99-issue-2-(2025).pdf) §§6.2.2.1–4 and the earlier claim ledger support the documented suitability questions; [NGED fast-track](https://connections.nationalgrid.co.uk/get-connected/solar-and-wind/fast-track-g99) supplies operator context, not a guarantee. [Nodemailer SMTP](https://nodemailer.com/smtp), [Vercel cron management](https://vercel.com/docs/cron-jobs/manage-cron-jobs) and [Supabase RLS](https://supabase.com/docs/guides/database/postgres/row-level-security) were checked for delivery/authentication/ownership implementation. Supabase changelog reviewed; no applicable migration-breaking change identified. Security advisor after the migrations: no findings.

Final visual correction verified: WebKit dropdowns now have deliberate48px targets at phone width; Chromium retains the same treatment. Legacy WordPress marketing entry routes are mapped explicitly in `vercel.json`: form-a/cart/plain checkout→apply, shop/product→offer, my-account/order-pay→contact, old order-received→unverified success. API routes and unknown paths are not captured by marketing redirects. The only historical WooCommerce record in the authenticated audit was a£1 processing record from February2025; its private recovery archive is retained. No WordPress deletion or hosting/mail cancellation is part of this cutover.
